...
QmyDom::QmyDom( bool heading )
{
   ...
   /// QmyDom::createProcessingInstruction()
   if (heading) {
   appendChild( header );
   }
}
...
void QmyDom::insertElemetFrag( QDomElement e , const QDomNodeList ex , QString newname )
{
    const int sizex = ex.size();
        bool check = (sizex > 0)? true:false;
           if ( check ) {
                       for (int x = 0; x < ex.count(); x++)
                       {
                           QDomNode node = ex.at(x);
                           if ( node.isElement() ) {
                               QDomElement xxx = importNode(node,true).toElement();
                               xxx.setTagName(newname);
                               e.appendChild(xxx);
                           }
                       }
           } else {
...
bool QmyDom::saveasHtml_Clean( const QString fullFileName )
{
    xml_unlink(fullFileName);
    /* save to a specific file */
    const QString xmlxx = QmyDom::toString();
    QString xcc = CleanTagAsHTML(xmlxx);
