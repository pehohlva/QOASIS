  ...
  while ( !element.isNull() ) {
    if ( element.tagName() == QLatin1String( "font-face" ) ) {
      ...
      mStyleInformation->addFontProperty( element.attribute( "name" ), property );
        ...
        qDebug() <<  "Unknown Tag= (" << element.tagName() << ") exist! You can make better to parse all Tag here file:line" << __FILE__ << ":" << __LINE__;
    }
...
StyleFormatProperty StyleParser::parseStyleProperty( QDomElement &parent )
{
  ...
  while ( !element.isNull() ) {
    if ( element.tagName() == QLatin1String( "paragraph-properties" ) ) {
      ...
      allfound = false;
      missing = element.tagName();
    }

    element = element.nextSiblingElement();
  }
    if (!allfound) {
        qDebug() <<  "Unknown Tag= (" << missing << ") exist! You can make better to parse all Tag here file:line" << __FILE__ << ":" << __LINE__;
     }
    return property;
}
...
double StyleParser::convertUnit( const QString &data )
{
  ...
  if ( data.endsWith( "pt" ) ) {
    ...
    if ( !data.isEmpty() ) {
      qDebug() <<  "Unknown unit! for= (" << data << ") exist! You can make better to parse all Unit here file:line" << __FILE__ << ":" << __LINE__;
