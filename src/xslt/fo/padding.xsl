			...
			<xsl:when test="($component/@text:anchor-type = 'frame') or ($component/@text:anchor-type = 'paragraph')">
				...
				<xsl:choose>
					<xsl:when test="$horz-pos = 'right'">
						<xsl:variable name="x-pos1" select="$parent-width-value - $image-width-value" />
						<xsl:variable name="x-pos2" select="$x-pos1 + $parent-padding-value + $parent-border-value" />
						<xsl:variable name="x-pos" select="$x-pos2 + $parent-border-right-value" />
						<!--xsl:variable name="x-pos" select="$x-pos - $parent-margin-left-value - $parent-margin-left-value" /-->

						<xsl:variable name="y-pos1" select="-0.062" />
						<xsl:variable name="y-pos2" select="$y-pos1 - $parent-padding-value - $parent-border-value" />
						<xsl:variable name="y-pos" select="$y-pos2 - $parent-padding-top-value - $parent-border-top-value" />
					...
					<xsl:otherwise>
						...
						<xsl:variable name="x-pos1" select="0.000" />
						<xsl:variable name="x-pos2" select="$x-pos1 - $parent-padding-value - $parent-border-value" />
						<xsl:variable name="x-pos" select="$x-pos2 - $parent-padding-left-value - $parent-border-left-value" />
						<!--xsl:variable name="x-pos" select="$x-pos - $parent-margin-left-value - $parent-margin-left-value" /-->
