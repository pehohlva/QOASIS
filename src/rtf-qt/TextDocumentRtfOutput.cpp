...
TextDocumentRtfOutput::TextDocumentRtfOutput(QTextDocument *document)
    : AbstractRtfOutput(), m_haveSetFont(false), m_document(document) {
...
TextDocumentRtfOutput::~TextDocumentRtfOutput() { delete m_cursor; }
...
void TextDocumentRtfOutput::startGroup() {
  ...
  QTextCharFormat charFormat =
      m_textCharFormatStack.top(); // inherit all current properties
...
void TextDocumentRtfOutput::insertPar() { m_cursor->insertBlock(); }

void TextDocumentRtfOutput::insertTab() { m_cursor->insertText("\t"); }
...
void TextDocumentRtfOutput::setParagraphPatternBackgroundColour(
...
void TextDocumentRtfOutput::setFont(const int fontIndex) {
  if (!m_fontTable.contains(fontIndex)) {
    qDebug() << "attempted to select fontIndex" << fontIndex
             << "not in the font table";
    return;
  }
...
void TextDocumentRtfOutput::insertFontTableEntry(FontTableEntry fontTableEntry,
                                                 quint32 fontTableIndex) {
  // qDebug() << "inserting font entry:" << fontTableIndex << "with name:" <<
  // fontTableEntry.fontName();
  m_fontTable.insert(fontTableIndex, fontTableEntry);
}

void TextDocumentRtfOutput::insertStyleSheetTableEntry(
    quint32 stylesheetTableIndex, StyleSheetTableEntry stylesheetTableEntry) {
  qDebug() << "inserting stylesheet entry:" << stylesheetTableIndex
           << "with name:" << stylesheetTableEntry.styleName();
...
void TextDocumentRtfOutput::resetCharacterProperties() {
  m_textCharFormatStack.top().setFontPointSize(
      12); // default of 24 "half-points"
...
void TextDocumentRtfOutput::setFontSuperscript() {
  m_textCharFormatStack.top().setVerticalAlignment(
...
void TextDocumentRtfOutput::createImage(const QImage &image,
