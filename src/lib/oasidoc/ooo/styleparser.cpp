...
StyleParser::StyleParser(const Document *document,
...
bool StyleParser::parseStyleFile() {
  ...
  if (!document.setContent(&source, &reader, &errorMsg, &errorLine,
                           &errorCol)) {
    ...
    if (element.tagName() == QLatin1String("meta")) {
      ...
      while (!child.isNull()) {
        if (child.tagName() == QLatin1String("generator")) {
          mStyleInformation->addMetaInformation(QStringLiteral("producer"),
                                                child.text(), i18n("Producer"));
        } else if (child.tagName() == QLatin1String("creation-date")) {
          const QDateTime dateTime =
              QDateTime::fromString(child.text(), Qt::ISODate);
          mStyleInformation->addMetaInformation(
          ...
          mStyleInformation->addMetaInformation(QStringLiteral("creator"),
                                                child.text(), i18n("Creator"));
        } else if (child.tagName() == QLatin1String("creator")) {
          mStyleInformation->addMetaInformation(QStringLiteral("author"),
                                                child.text(), i18n("Author"));
        } else if (child.tagName() == QLatin1String("date")) {
          const QDateTime dateTime =
              QDateTime::fromString(child.text(), Qt::ISODate);
          mStyleInformation->addMetaInformation(
              QStringLiteral("modificationDate"),
              QLocale().toString(dateTime, QLocale::LongFormat),
              i18n("Modified"));
        }
...
bool StyleParser::parseDocumentCommonAttrs(QDomElement &) { return true; }
...
bool StyleParser::parseFontFaceDecls(QDomElement &parent) {
  ...
  while (!element.isNull()) {
    if (element.tagName() == QLatin1String("font-face")) {
      ...
      mStyleInformation->addFontProperty(
          element.attribute(QStringLiteral("name")), property);
    } else {
...
bool StyleParser::parseStyles(QDomElement &) { return true; }

bool StyleParser::parseMasterStyles(QDomElement &parent) {
  QDomElement element = parent.firstChildElement();
  while (!element.isNull()) {
    if (element.tagName() == QLatin1String("master-page")) {
      mStyleInformation->addMasterLayout(
          element.attribute(QStringLiteral("name")),
          element.attribute(QStringLiteral("page-layout-name")));
      if (!mMasterPageNameSet) {
        mStyleInformation->setMasterPageName(
            element.attribute(QStringLiteral("name")));
...
bool StyleParser::parseAutomaticStyles(QDomElement &parent) {
  ...
  while (!element.isNull()) {
    if (element.tagName() == QLatin1String("style")) {
      ...
      mStyleInformation->addStyleProperty(
          element.attribute(QStringLiteral("name")), property);
      ...
      while (!child.isNull()) {
        if (child.tagName() == QLatin1String("page-layout-properties")) {
          ...
          mStyleInformation->addPageProperty(
              element.attribute(QStringLiteral("name")), property);
        }
      ...
      mStyleInformation->addListProperty(
          element.attribute(QStringLiteral("name")), property);
      ...
      mStyleInformation->addStyleProperty(
          element.attribute(QStringLiteral("family")), property);
    } else {
      qDebug("unknown tag %s", qPrintable(element.tagName()));
    }
...
StyleFormatProperty StyleParser::parseStyleProperty(QDomElement &parent) {
  ...
  property.setParentStyleName(
      parent.attribute(QStringLiteral("parent-style-name")));
  property.setFamily(parent.attribute(QStringLiteral("family")));
  if (parent.hasAttribute(QStringLiteral("master-page-name"))) {
    property.setMasterPageName(
        parent.attribute(QStringLiteral("master-page-name")));
    if (!mMasterPageNameSet) {
      mStyleInformation->setMasterPageName(
          parent.attribute(QStringLiteral("master-page-name")));
      mMasterPageNameSet = true;
  ...
  while (!element.isNull()) {
    if (element.tagName() == QLatin1String("paragraph-properties")) {
      const ParagraphFormatProperty paragraphProperty =
          parseParagraphProperty(element);
      ...
      const TableColumnFormatProperty tableColumnProperty =
          parseTableColumnProperty(element);
      property.setTableColumnFormat(tableColumnProperty);
    } else if (element.tagName() == QLatin1String("table-cell-properties")) {
      const TableCellFormatProperty tableCellProperty =
          parseTableCellProperty(element);
...
StyleParser::parseParagraphProperty(QDomElement &parent) {
  ParagraphFormatProperty property;

  property.setPageNumber(
      parent.attribute(QStringLiteral("page-number")).toInt());
  ...
  property.setWritingMode(
      map[parent.attribute(QStringLiteral("writing-mode"))]);
  ...
  if (parent.hasAttribute(QStringLiteral("text-align"))) {
    property.setTextAlignment(alignMap[parent.attribute(
        QStringLiteral("text-align"), QStringLiteral("left"))]);
  }
  ...
  const QString colorText =
      parent.attribute(QStringLiteral("background-color"));
...
PageFormatProperty StyleParser::parsePageProperty(QDomElement &parent) {
  ...
  property.setBottomMargin(
      convertUnit(parent.attribute(QStringLiteral("margin-bottom"))));
  property.setLeftMargin(
      convertUnit(parent.attribute(QStringLiteral("margin-left"))));
  property.setTopMargin(
      convertUnit(parent.attribute(QStringLiteral("margin-top"))));
  property.setRightMargin(
      convertUnit(parent.attribute(QStringLiteral("margin-right"))));
  property.setWidth(
      convertUnit(parent.attribute(QStringLiteral("page-width"))));
  property.setHeight(
      convertUnit(parent.attribute(QStringLiteral("page-height"))));
...
ListFormatProperty StyleParser::parseListProperty(QDomElement &parent) {
  ...
  while (!element.isNull()) {
    if (element.tagName() == QLatin1String("list-level-style-number")) {
      ...
      int level = element.attribute(QStringLiteral("level")).toInt();
      property.addItem(level, convertUnit(element.attribute(
                                  QStringLiteral("space-before"))));
    }
...
StyleParser::parseTableColumnProperty(QDomElement &parent) {
  TableColumnFormatProperty property;

  const double width =
      convertUnit(parent.attribute(QStringLiteral("column-width")));
...
StyleParser::parseTableCellProperty(QDomElement &parent) {
  ...
  if (parent.hasAttribute(QStringLiteral("align")) &&
      parent.hasAttribute(QStringLiteral("vertical-align"))) {
    property.setAlignment(
        map[parent.attribute(QStringLiteral("align"))] |
        map[parent.attribute(QStringLiteral("vertical-align"))]);
...
#define CC_TO_POINT(cc) ((cc)*12.840103)
  ...
  double points = 0;
  if (data.endsWith(QLatin1String("pt")) || data.endsWith(QLatin1String("px")) ) {
