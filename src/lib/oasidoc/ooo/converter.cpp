...
QTextDocument *Converter::convert(const QString &fileName , QString &html) {
  ...
  HtmlDriver *htmlcore = new HtmlDriver();
  ...
  htmlcore->~HtmlDriver(); /// clean cache dir..
...
///// draw:name  ODTDEBUG() << "XXXXXXXXXXXXXXX  init:" << __FUNCTION__;
...
bool Converter::exist_image( const QString & name ) {
