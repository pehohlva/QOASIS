...
DocSession *DocSession::st_ = 0;

DocSession *DocSession::instance() {
  if (st_ == 0) {
    st_ = new DocSession();
  }
  return st_;
...
DocSession::DocSession() : pdev(new QPrinter(QPrinter::ScreenResolution)) {
  ...
  CDLabel.RealSize =
      QSizeF(MM_TO_POINT(220), MM_TO_POINT(220)); //////CDLabel.G_regt.size();
  ...
  FormatRegister(QT_TR_NOOP("Executive (7.5 x 10 inches, 191 x 254 mm)"),
  FormatRegister(QT_TR_NOOP("Folio (210 x 330 mm)"), QPrinter::Folio);
  FormatRegister(QT_TR_NOOP("Ledger (432 x 279 mm)"), QPrinter::Ledger);
  FormatRegister(QT_TR_NOOP("Legal (8.5 x 14 inches, 216 x 356 mm)"),
  FormatRegister(QT_TR_NOOP("Letter (8.5 x 11 inches, 216 x 279 mm)"),
  FormatRegister(QT_TR_NOOP("Tabloid (279 x 432 mm)"), QPrinter::Tabloid);
  FormatRegister(QT_TR_NOOP("US Common #10 Envelope (105 x 241 mm)"),
  AppendPaper(CDLabel, true);
  current_Page_Format = history_page_norms[1];
}

void DocSession::clearKey() { uniqueid.clear(); }
bool DocSession::validKey(const QString idkei) const {
...
QString DocSession::validateUnique(const QString idkei) {
  ...
  if (validKey(idkei)) {
    ...
    //////////////qDebug() << "### unable / unique key  / to handle->"  <<
    /// idkei;
    return QString(idkei);
  }
}

qreal DocSession::maximumPage() {
  qreal largo = 500;
...
bool DocSession::is_OnChain(M_PageSize e) {
  bool iifound = false;
...
bool DocSession::coreSave(M_PageSize e) {
  bool cansave = false;
...
void DocSession::FormatRegister(const QString txt, QPrinter::PageSize pp) {
...
void DocSession::AppendPaper(M_PageSize cur, bool enableregion) {
...
M_PageSize DocSession::FindPagePsize(const QRect paper) {
  QMapIterator<int, M_PageSize> i(mpages());
  while (i.hasNext()) {
    i.next();
