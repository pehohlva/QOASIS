...
{
    //qDebug() <<"writing out style" << name <<" display-name=" << m_attributes["style:display-name"] <<" family=" << m_familyName;
    writer->startElement(elementName);
    ...
    if (!m_defaultStyle) {
        ...
        if (!m_parentName.isEmpty()) {
            ...
            if (parentStyle && m_familyName.isEmpty()) {
                ...
                //qDebug() <<"Got familyname" << m_familyName <<" from parent";
            }
    ...
    if (!m_familyName.isEmpty())
        ...
        if (qstrcmp(elementName, "style:style") == 0)
           qDebug() << "User style " << name << " is without family - invalid. m_type=" << m_type;
    }
    ...
    if (!m_properties[i].isEmpty() || !m_properties[defaultPropertyType].isEmpty()) {  //// hystory 2010
        if (createPropertiesTag)
...
{
    int i = DefaultType;
    qDebug() << m_properties[i].count() << " properties.";
    for (QMap<QString, QString>::ConstIterator it = m_properties[i].constBegin(); it != m_properties[i].constEnd(); ++it) {
        qDebug() << "" << it.key() << " =" << it.value();
    }
    i = TextType;
    qDebug() << m_properties[i].count() << " text properties.";
    for (QMap<QString, QString>::ConstIterator it = m_properties[i].constBegin(); it != m_properties[i].constEnd(); ++it) {
        qDebug() << "" << it.key() << " =" << it.value();
    }
    i = ParagraphType;
    qDebug() << m_properties[i].count() << " paragraph properties.";
    for (QMap<QString, QString>::ConstIterator it = m_properties[i].constBegin(); it != m_properties[i].constEnd(); ++it) {
        qDebug() << "" << it.key() << " =" << it.value();
    }
    /*
    i = ChildElement;
    qDebug() << m_properties[i].count() << " child elements.";
    for (QMap<QString, QString>::ConstIterator it = m_properties[i].constBegin(); it != m_properties[i].constEnd(); ++it) {
        qDebug() << "" << it.key() << " =" << it.value();
    }
    */
    qDebug() << m_attributes.count() << " attributes.";
    for (QMap<QString, QString>::ConstIterator it = m_attributes.constBegin(); it != m_attributes.constEnd(); ++it) {
        qDebug() << "" << it.key() << " =" << it.value();
    }
    qDebug() << m_maps.count() << " maps.";
    for (int i = 0; i < m_maps.count(); ++i) {
        qDebug() << "map" << i << ":";
